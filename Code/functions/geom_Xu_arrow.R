...
geom_Xu_arrow <- function(mapping = NULL,
                          data = NULL,
                          stat = "arrow",
                          position = "identity",
                          ...,
...
#' @export
#' @rdname geom_arrow
geom_vector <- function(mapping = NULL,
                        data = NULL,
                        stat = "arrow",
                        position = "identity",
                        ...,
...
GeomArrow <- ggplot2::ggproto(
  ...
  draw_panel = function(data,
    ...
    if ("simpleUnit" %in% class(unit(1, "mm"))) {
      arrow$length <- arrow$length * data$norm_mag
    } else {
    ...
    if (preserve.dir == FALSE) {
      ...
      data2$x <-
        with(data, x + dx / 10000)
      data2$y <-
        with(data, y + dy / 10000)
      
      coords <-
        coord$transform(data, panel_scales)
      coords2 <-
        coord$transform(data2, panel_scales)
      
      coords$xend <- coords2$x
      coords$yend <- coords2$y
      coords$dx <-
        with(coords, xend - x) / 100
      coords$dy <-
        with(coords, yend - y) / 100
  ...
  setup_data = function(data, params) {
    ...
    if (is.null(data$mag) |
      ...
      if (is.null(data$dx) | is.null(data$dy)) {
        stopf("stat_arrow needs wither 'mag' and 'angle' or 'dx' and 'dy'.",
              call. = FALSE)
      }
      data$mag <-
        with(data, Mag(dx, dy))
      data$angle <-
        with(data, atan2(dy, dx) * 180 / pi)
    } else {
      # Turn into mathematical angle
      data$angle <-
        params$start - data$angle * params$direction
      data$dx <-
        with(data, mag * cos(angle * pi / 180))
      data$dy <-
        with(data, mag * sin(angle * pi / 180))
    }
    
    data <-
      subset(
        data,
        x %in% JumpBy(sort(unique(x)), params$skip.x + 1) &
          y %in% JumpBy(sort(unique(y)), params$skip.y + 1) &
          mag >= params$min.mag
      )

    # Change 2 --------------------------------------------------------
    per.0.95 <- quantile(data$mag,probs = 0.95)
    data$norm_mag <- data$mag / per.0.95
    data$norm_mag[which(data$norm_mag>1)] = 1
    data
  },
  compute_panel = function(self,
    ...
    stats <- mapply(function(new, old) {
      if (plyr::empty(new))
        return(data.frame())
      unique <-
        ggplot2:::uniquecols(old)
      missing <-
        !(names(unique) %in% names(new))
      cbind(new,
    ...
    if (preserve.dir == FALSE) {
      if (scales$x$is_discrete() | scales$y$is_discrete()) {
        warningf("The use of preserve.dir = FALSE with discrete scales is not recommended.",
                 call. = FALSE)
      }
      
      if (scales$x$scale_name == "date" |
          scales$x$scale_name == "date") {
        warningf("The use of preserve.dir = FALSE with date scales is not recommended.",
                 call. = FALSE)
      }
    }
