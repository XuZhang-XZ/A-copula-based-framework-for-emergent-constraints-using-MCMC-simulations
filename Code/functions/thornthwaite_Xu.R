...
thornthwaite_Xu  = function (Tave, lat, na.rm = FALSE) {
  
  ## daylight hours L
  tanLat <- tan(lat/57.2957795)
  tanDelta <- rep(c(-0.37012566, -0.23853358, -0.04679872, 0.16321764, 
                0.32930908, 0.40677729, 0.3747741, 0.239063, 0.04044485, 
                -0.16905776, -0.33306377, -0.40743608),1000)[1:length(Tave)]
  ...
  ## Number of Days
  days <- rep(c(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31),1000)[1:length(Tave)]
  
  ## heat index I
  temp = matrix(Tave,ncol = 12, byrow = TRUE)
  temp_mean = colMeans(temp)
  temp_mean[temp_mean < 0] <- 0
  I = sum((temp_mean/5)^1.514)
  
  ## Alpha
  alpha <- 6.75e-07 * I^3 - 7.71e-05 * I^2 + 0.01792 * I + 0.49239
  
  ## PE
  if(is.na(I)) {
    PE = rep(NA, length(Tave))
    return(PE)
  }
  if(I == 0) {
    PE = rep(0, length(Tave))
  }
  if(I > 0) {
    Tave1 = Tave
    Tave1[Tave1 < 0] <- 0
    PE = 16 * (L/12) * (days / 30) * (10 * Tave1 / I) ^ alpha
  }
...
thornthwaite_Xu_old = function (Tave, lat, na.rm = FALSE) {
